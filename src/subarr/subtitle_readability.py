...
class ReadabilityReport:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "counts": self.counts,
            "issues": [
                {"cue": i.cue_index, "kind": i.kind, "severity": i.severity, "detail": i.detail}
                for i in self.issues
            ],
        }
    ...
    for n, c in enumerate(cues):
        if c.line_count > MAX_LINES:
            report.issues.append(
                CueIssue(c.index, "lines", "warn", f"{c.line_count} lines (max {MAX_LINES})")
            )
        ...
        if 0 < c.duration_s < MIN_DURATION_S:
            report.issues.append(
                CueIssue(
                    c.index, "too_short", "warn", f"{c.duration_s:.2f}s on screen (min {MIN_DURATION_S:.2f}s)"
                )
            )
        elif c.duration_s > MAX_DURATION_S:
            report.issues.append(
                CueIssue(
                    c.index, "too_long", "warn", f"{c.duration_s:.1f}s on screen (max {MAX_DURATION_S:.0f}s)"
                )
            )
        if n + 1 < len(cues) and c.end_ms > cues[n + 1].start_ms:
            report.issues.append(
                CueIssue(c.index, "overlap", "critical", f"overlaps cue {cues[n + 1].index}")
            )
    return report
