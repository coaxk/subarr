    ...
    for src_key, (settings_attr, coerce) in mapping.items():
        if src_key in progress and progress[src_key]:
            ...
            try:
                ...
                # Config-loss fix (field report): persist the wizard value below
                # env so a UI-configured integration survives a restart. The
                # original only patched the running Settings (the v1.0 docstring
                # above assumed the operator would pin env vars), so wizard-only
                # users silently lost their arr/plex/bazarr creds on the next
                # container recreate. #112's store persists below env.
