...
One automatic database migration (`033`), applied on first start. No config changes.
...
Four fixes. No config changes and no migration. Two came out of the review of the whole-show paging feature in 2.7.0, one was reported by @Jorman, and one is the test harness that made the first two fixable at the level where they had been breaking.

### Fixed
- **Bulk verify failures were counted and then thrown away by the batch's own completion (#515).** Assign a language to 400 episodes while your Sonarr API key has expired and every request fails. The progress bar read "400 failed" for one frame, then vanished, the list refreshed, and the same 400 rows sat there with no explanation. The count was only ever drawn inside the selection bar, and finishing the batch cleared the selection and unmounted the bar in the same instant. Failed rows now stay ticked with the bar still open, a banner outside the bar names the count and the first files, and Apply is one click away once the cause is fixed. There is also a Stop button for a batch that is now minutes long rather than seconds, and the browser warns before you navigate away mid-batch, because a click on Coverage used to silently drop the rest of the queue.
- **A verification could report success while Sonarr was never updated (#516).** Each verification is saved locally and then, if you have propagation turned on, pushed to Sonarr so Bazarr sees the correct audio language. That push can fail five different ways, and every one of them was answered with a plain success. Rows left Review looking done while Sonarr still recorded English and Bazarr stayed blind, which is precisely the loop this feature exists to close. Every failure now carries its reason, the Review banner says "verified locally, but Sonarr was not updated for N files" with one line per cause rather than one per file, and the single-file dialog says the same before it closes. Along the way: Sonarr's language table was documented as cached and was not, so a 400-file batch fetched it 400 times; it is now cached properly. And Bazarr's full metadata sync was triggered once per file, which was fine under the old 200-file page cap and not fine without it; a burst now costs two syncs, one immediately and one after the last file. This only affects installs with `SONARR_PROPAGATE_AUDIO_LANG=1`; it is off by default.
- **Opening the Logs page and leaving it flooded the log and stalled the app (#526).** The Logs page follows subgen's container output. When you navigated away, the reader kept running until subgen itself restarted, queueing every line for a listener that was gone; subgen's progress output is chatty enough to park thousands of those a second. The result was a wall of `Task was destroyed but it is pending` in subarr's own log and a stalled event loop, reported by @Jorman with the log that showed exactly this. The reader is now shut down the moment the page is closed, and under backpressure it drops lines rather than piling them up. Also from that report: the per-language tuning panel said "HTTP 503" when it could not read subgen's compose file. It now says why. The usual reason is that the file is readable only by root while subarr runs as your `PUID`, so a root shell inside the container can read what the app cannot, and it tells you both fixes.
- **The Review page could be fully disconnected from its own feature and every frontend test would still pass (#517).** The tests covered the helpers thoroughly and the wiring between them not at all, which is how #515 and #516 were possible in the first place. There is now a first component-level test that renders the real page, and the two fixes above are tested through it.
...
Seven fixes and one security change. No config changes and no migration. Three of them were reported by @IdahoRC from a live Unraid deployment, each with the cause already traced; one by @xiaden, who also wrote the fix.

The three related fixes are all cases where subarr looked at, or listened to, the wrong thing: it could attach one file's audio details to another file, score a subtitle that was not the one just generated, and throw away an audio language you had explicitly confirmed. None of them corrupted anything, and all three corrected themselves once the right data was read, which is part of why they lasted.
...
### Fixed
- **Coverage could show one file's audio details against a different file (#497).** subarr looks up the audio and subtitle details of a movie using the folder it sits in. If you replaced or renamed a movie and the details of the old file were still on record under that folder, subarr could attach the old file's information to the new one and treat it as confirmed. The result was a row that looked fully checked while describing a file that is no longer there. subarr now requires the details to belong to the exact file it is looking at, and otherwise leaves the row waiting to be checked properly. Episodes were never affected: they have always been matched by filename.
- **An English audio language you verified yourself was silently discarded (#498).** When a file carries no audio language information, because the encoder never recorded one, you can confirm the language by hand. If you confirmed English, that confirmation was thrown away and never reached the transcription service, so the file was handled using the missing information you had just corrected. The reason is genuinely counter-intuitive: telling the service the audio is English does not exempt the file from the rule that skips English audio, it makes the file match that rule. On a setup configured to skip English audio, passing the confirmation along would have stopped the file being transcribed at all, which is why it was being withheld from everyone. subarr now asks the transcription service which languages it skips, and passes your confirmation along whenever doing so cannot cause the file to be skipped. Confirming any other language worked before and is unchanged. This needs subarr-subgen 2026.08.1-r8 or newer to take effect; on older versions subarr cannot tell what the service skips and behaves exactly as it did before.
- **The subtitle quality report could grade the wrong subtitle file (#499).** After a subtitle is generated, subarr reads it back and scores it. It looked for a specific filename and, failing that, took whichever matching file the disk happened to return first. If your transcription service is configured to include its own name and the model in the filename, the file it had just written was not the one being looked for, so the report could describe an older or unrelated subtitle sitting beside the video. The reporter measured it: a 1,317-line subtitle had just been produced and a 763-line file was being graded instead. subarr now recognises the naming the service actually uses, prefers the English one, never grades a forced-subtitle file as if it were a full transcript, and no longer depends on the order the disk returns files.
- **The Review page could show a different set of rows each time you paged through it (#448).** Rows were ordered by however the underlying scan happened to produce them, and that order is not stable, so paging could show the same title twice or skip one entirely. They are now sorted before the page is cut, with TV before films and titles in alphabetical order. Contributed by @xiaden.
- **subarr's advice when it could not use the full transcription service was wrong (#59).** When the service is missing features subarr needs, subarr says so and suggests switching to the subarr-subgen image. For one reporter that advice was impossible to follow, because they were already using it. What subarr detects is that the running code lacks those features, which is not the same as the image lacking them: a setting carried over from a previous setup was making the container replace its own code at startup. The message now names that cause and the settings that produce it, so the next person is pointed at the actual problem instead of at something they have already done.
- **Onboarding progress was recorded against the wrong step numbers (#480).** The internal numbering used to record how far through setup an install had reached had drifted out of step with the wizard, so the anonymous progress counts described the wrong screens. This affects only the aggregate figures used to find where setup goes wrong; nothing about the wizard itself changes, and no additional information is collected.
...
- **Clearing out stale queue and Review entries after a rename (#453, #469).** Renaming files leaves entries wedged in Review pointing at paths that no longer exist, and pending queue rows that can never be satisfied. Both now have a cleanup you can reach from the UI: it detects on load, tells you the count, shows the full list on request, and requires a second explicit click. Each states plainly what it removes, which is subarr's own bookkeeping and never a media or subtitle file. One reporter had 110 wedged rows.
- **Search and pagination on Review and Aftercare (#449).** Contributed by @xiaden. Both lists are now searched and paged server-side rather than loading everything and filtering in the browser.
...
- **Forced subtitles were named so that Bazarr could not see them (#475).** The default was `.forced.en.srt`. Bazarr reads `.en.forced.srt`, so every forced sidecar subarr wrote was invisible to it and the episode stayed on the wanted list. The default is corrected, and the existing form is still recognised so an install that already has files in the old layout does not end up with duplicates beside them.
- **A skipped queue row took 60 seconds to clear instead of milliseconds (#468).** subgen answers `skipped` immediately and subarr was throwing that answer away, then inferring the same fact a minute later from the row's absence. Each wedged row held a feeder slot for the whole minute. One reporter hit this 110 times in a single scan.
- **Five background tasks were invisible until their first success (#157).** `telemetry`, `data-persistence`, `data-network-fs`, `db-integrity` and `single-process` recorded outcomes but were never registered, so they were missing from the Health roster entirely until they had already succeeded once. A loop that died during startup, or never ran at all, therefore showed as nothing rather than as a problem.
...
### Dependencies
- Routine dev-dependency and GitHub Actions updates via dependabot.
