...
def test_upsert_and_get(tmp_path):
    s = _store(tmp_path)
    s.upsert(canonical_path="TV/X/a.mkv", tag_lang="da", detected_lang="nl",
             status="mislabel", languages_heard=["nl"], n_agreeing=3,
             n_total=3, mtime=100.0)
    f = s.get("TV/X/a.mkv")
...
def test_upsert_replaces_existing(tmp_path):
    ...
    s.upsert(canonical_path="TV/X/a.mkv", tag_lang="da", detected_lang=None,
             status="undetermined", languages_heard=[], n_agreeing=0,
             n_total=0, mtime=100.0)
    s.upsert(canonical_path="TV/X/a.mkv", tag_lang="da", detected_lang="nl",
             status="mislabel", languages_heard=["nl"], n_agreeing=3,
             n_total=3, mtime=200.0)
    f = s.get("TV/X/a.mkv")
...
def test_list_findings_only_actionable_newest_first(tmp_path):
    s = _store(tmp_path)
    s.upsert(canonical_path="agrees.mkv", tag_lang="en", detected_lang="en",
             status="agrees", languages_heard=["en"], n_agreeing=3, n_total=3, mtime=1.0)
    s.upsert(canonical_path="mislabel.mkv", tag_lang="da", detected_lang="nl",
             status="mislabel", languages_heard=["nl"], n_agreeing=3, n_total=3, mtime=1.0)
    s.upsert(canonical_path="bilingual.mkv", tag_lang="sr", detected_lang="sr",
             status="bilingual", languages_heard=["en", "sr"], n_agreeing=2, n_total=3, mtime=1.0)
...
def test_count_by_status_and_clear(tmp_path):
    s = _store(tmp_path)
    s.upsert(canonical_path="a.mkv", tag_lang="en", detected_lang="en",
             status="agrees", languages_heard=[], n_agreeing=3, n_total=3, mtime=1.0)
    s.upsert(canonical_path="b.mkv", tag_lang="da", detected_lang="nl",
             status="mislabel", languages_heard=["nl"], n_agreeing=3, n_total=3, mtime=1.0)
...
def test_parse_robust_detect_unanimous():
    resp = {"aggregate": {"language": "nl", "n_agreeing": 3, "n_total": 3},
...
def test_parse_robust_detect_drops_und():
    resp = {"aggregate": {"language": "und", "n_agreeing": 1, "n_total": 3},
...
def _unanimous(lang, n=3):
    return {"aggregate": {"language": lang, "n_agreeing": n, "n_total": n},
...
def _split(langs):
    ...
    return {"aggregate": {"language": top, "n_agreeing": n, "n_total": len(langs)},
...
async def test_walker_undetermined_when_no_detection(tmp_path):
    ...
    w = AudioAuditWalker(subgen, s, worklist=lambda: [("x.mkv", "en", 10.0)],
                         to_subgen=_identity)
    await (await w.start(), w._task)[-1]
...
async def test_walker_multitrack_from_probe(tmp_path):
    ...
    w = AudioAuditWalker(subgen, s, worklist=lambda: [("mt.mkv", "de", 10.0)],
...
async def test_walker_resumable_skips_matching_mtime(tmp_path):
    ...
    s.upsert(canonical_path="seen.mkv", tag_lang="en", detected_lang="en",
             status="agrees", languages_heard=["en"], n_agreeing=3, n_total=3, mtime=10.0)
...
async def test_walker_reaudits_when_mtime_changed(tmp_path):
    from subarr.audio_audit import AudioAuditWalker
    s = _store(tmp_path)
    s.upsert(canonical_path="f.mkv", tag_lang="en", detected_lang="en",
             status="agrees", languages_heard=["en"], n_agreeing=3, n_total=3, mtime=10.0)
    subgen = _FakeSubgen({"f.mkv": _unanimous("nl")})
    # mtime changed (50.0 != 10.0) → re-audit
    w = AudioAuditWalker(subgen, s, worklist=lambda: [("f.mkv", "da", 50.0)],
                         to_subgen=_identity)
    await (await w.start(), w._task)[-1]
...
async def test_walker_pauses_while_busy_no_detect(tmp_path, monkeypatch):
    ...
    busy = {"v": True}
    w = aa.AudioAuditWalker(subgen, s, worklist=lambda: [("x.mkv", "da", 10.0)],
                            busy_check=lambda: busy["v"], to_subgen=_identity)
    state = await w.start()
...
async def test_walker_double_start_guarded(tmp_path, monkeypatch):
    ...
    w = aa.AudioAuditWalker(
        subgen, s, worklist=lambda: [("a.mkv", "en", 1.0), ("b.mkv", "en", 1.0)],
        to_subgen=_identity)
    await w.start()
...
async def test_walker_per_file_exception_recorded_and_continues(tmp_path, monkeypatch):
    ...
    w = aa.AudioAuditWalker(
        subgen, s, worklist=lambda: [("bad.mkv", "en", 1.0), ("good.mkv", "da", 1.0)],
        to_subgen=_identity)
    state = await w.start()
    await w._task
    assert state.status == "done"
