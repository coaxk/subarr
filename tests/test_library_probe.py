...
def _seed_probe(app_with_stub, canonical: str, sub_kind: str = "full"):
    ...
    if sub_kind == "full":
        ...
        pr.subtitles.append(SubtitleStream(
            index=0, language="eng", codec="subrip", title="English (SDH)",
            default=False, forced=False, sdh=True, commentary=False,
        ))
    elif sub_kind == "forced":
        pr.subtitles.append(SubtitleStream(
            index=0, language="eng", codec="subrip", title="English Forced",
            default=False, forced=True, sdh=False, commentary=False,
        ))
    elif sub_kind == "commentary":
        pr.subtitles.append(SubtitleStream(
            index=0, language="eng", codec="subrip", title="Director Commentary",
            default=False, forced=False, sdh=False, commentary=True,
        ))
    # 'none' → no sub stream
    app_with_stub.app.state.probe_store.upsert(
        canonical_path=canonical, mtime=st.st_mtime, size=st.st_size, result=pr,
    )
...
def _bazarr_with_two_episodes(req: httpx.Request) -> httpx.Response:
    ...
    if p == "/api/episodes/wanted":
        return httpx.Response(200, json={"data": [
            {"seriesTitle": "Covered", "episode_number": "1x1",
             "episodeTitle": "Has EN", "missing_subtitles": [{"name": "English", "code2": "en"}],
             "sonarrSeriesId": 42, "sonarrEpisodeId": 9001, "tags": []},
            {"seriesTitle": "Gap", "episode_number": "1x1",
             "episodeTitle": "No EN", "missing_subtitles": [{"name": "English", "code2": "en"}],
             "sonarrSeriesId": 43, "sonarrEpisodeId": 9002, "tags": []},
        ]})
...
def _sonarr_two_series(req: httpx.Request) -> httpx.Response:
    ...
    if req.url.path == "/api/v3/series":
        return httpx.Response(200, json=[
            {"id": 42, "title": "Covered", "monitored": True,
             "path": "/data/Media/TV/Covered",
             "originalLanguage": {"id": 1, "name": "English"}, "tags": []},
            {"id": 43, "title": "Gap", "monitored": True,
