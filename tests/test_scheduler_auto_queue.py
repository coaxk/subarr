...
def test_evaluate_skips_unverified_rows():
    ...
    rules = AutoQueueRules(mode=MODE_AUTO_RULES, min_score=0, deny_languages=[], require_monitored=False)
...
def test_evaluate_auto_rules_min_score():
    ...
    rules = AutoQueueRules(mode=MODE_AUTO_RULES, min_score=200, deny_languages=[], require_monitored=False)
    decisions = evaluate(items, rules)
...
def test_evaluate_caps_max_per_run():
    ...
    rules = AutoQueueRules(
        mode=MODE_AUTO_RULES, min_score=0, max_per_run=3, deny_languages=[], require_monitored=False
    )
    decisions = evaluate(items, rules)
...
def test_evaluate_deny_languages():
    ...
    rules = AutoQueueRules(
        mode=MODE_AUTO_RULES, min_score=0, deny_languages=["English"], require_monitored=False
    )
    decisions = evaluate(items, rules)
...
def test_schedule_patch(app_with_stub):
    r = app_with_stub.patch(
        "/api/schedule/coverage_walk", json={"enabled": True, "kind": "interval", "interval_minutes": 120}
    )
    assert r.status_code == 200
    body = r.json()
...
def test_rules_put(app_with_stub):
    r = app_with_stub.put(
        "/api/schedule/rules", json={"mode": "auto_rules", "min_score": 1000, "deny_languages": []}
    )
    assert r.status_code == 200
    body = r.json()
